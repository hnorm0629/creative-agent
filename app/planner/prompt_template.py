...
def creative_plan_trends_prompt(user_input: str) -> str:
    """
    Returns a full prompt string instructing the LLM to generate a vivid,
    structured creative plan for a short-form video based on the user's input.
    """
...
Given the following user brief, generate a highly imaginative and vivid creative plan for a video idea.
The goal is to surprise, delight, and push boundaries of what's expected. The final creative plan must
touch on current trending topics of this past week, whether it be in characters, scene ideas, or dialogue.
Make the output relevant to today's news and happenings, particularly viral topics.
